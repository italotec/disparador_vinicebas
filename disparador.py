...
# ===== Linha de parâmetro (equivalente ao ParamRow) =====
class ParamRow(ttk.Frame):
    def __init__(self, master, get_csv_headers_callable, *args, **kwargs):
        ...
        self.param_name_entry.insert(0, "")
...
def build_parameters_from_rows(rows, lead):
    ...
    for row in rows:
        ...
        if not m:
            continue
        if isinstance(m["parameter_name"], tuple):  # __RANDOM__
...
def modo_envio(bm_obj, csv_df, params_rows, random_mode=False, log_callback=None):
    ...
    # 1 thread (como original). Pode ajustar max_workers se desejar.
...
# ===== Frame rolável (equivalente ao CTkScrollableFrame) =====
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Sidebar
        self.sidebar = ttk.Frame(self, style="Card.TFrame")
        ...
        self.bm_menu = ttk.Combobox(self.sidebar, values=list(self.bms.keys()), textvariable=self.bm_menu_var, state="readonly")
        self.bm_menu.pack(fill="x", pady=2)
        self.bm_menu.bind("<<ComboboxSelected>>", self._select_bm)

        ttk.Button(self.sidebar, text="Cadastrar BM", style="Accent.TButton", command=self._cadastrar_bm).pack(fill="x", pady=(8, 6))
        ttk.Button(self.sidebar, text="Recarregar BMs", command=self._refresh_bms).pack(fill="x", pady=(0, 12))

        ttk.Label(self.sidebar, text="Arquivo CSV", style="Label.TLabel").pack(anchor="w", pady=(6, 2))
        ttk.Button(self.sidebar, text="Selecionar CSV", command=self._pick_csv).pack(fill="x", pady=2)

        self.csv_label_var = tk.StringVar(value="Nenhum arquivo selecionado")
        ttk.Label(self.sidebar, textvariable=self.csv_label_var, style="Hint.TLabel", wraplength=220, justify="left").pack(anchor="w", pady=(4, 12))

        ttk.Button(self.sidebar, text="Enviar (Normal)", command=lambda: self._start_envio(False)).pack(fill="x", pady=(4, 6))
        ttk.Button(self.sidebar, text="Enviar (Aleatório)", command=lambda: self._start_envio(True)).pack(fill="x", pady=(0, 10))

        # Área de parâmetros (card)
        self.param_frame = ttk.Frame(self, padding=10, style="Card.TFrame")
        self.param_frame.grid(row=1, column=1, sticky="nsew", padx=10, pady=(0, 10))
        self.param_frame.grid_columnconfigure(0, weight=1)

        header = ttk.Frame(self.param_frame, style="Card.TFrame")
        ...
        self.rows_container = ScrollableFrame(self.param_frame, height=280)
        self.rows_container.grid(row=1, column=0, sticky="nsew", pady=(10, 10))

        actions = ttk.Frame(self.param_frame, style="Card.TFrame")
        ...
        self.log_card.grid(row=2, column=0, columnspan=2, sticky="nsew", padx=10, pady=(0, 10))
        self.log_text = tk.Text(self.log_card, height=10, wrap="word", bg="#0b1220", fg="#e6e6e6", insertbackground="#e6e6e6", relief="flat")
    ...
    def _setup_style_dark(self):
        ...
        self.style.map("Card.TFrame", background=[("active", card)])
        ...
        self.style.configure("Accent.TButton", background=primary, foreground="#ffffff", padding=8)
        self.style.map("Accent.TButton",
                       background=[("active", primary_hover)])

        # Combobox / Entry
        field_opts = dict(fieldbackground=panel, background=panel, foreground=fg)
        self.style.configure("TCombobox", **field_opts, selectforeground=fg, selectbackground=panel, padding=6)
        self.style.configure("TEntry", **field_opts, padding=6)
        # Checkbutton
    ...
    def _add_param_row(self):
        row = ParamRow(self.rows_container.inner, get_csv_headers_callable=self._get_csv_headers)
        row.grid(sticky="ew", padx=6, pady=4)
        return row

    def _fill_example_rows(self):
        # Limpa
        for c in list(self.rows_container.inner.winfo_children()):
            if isinstance(c, ParamRow):
                c.destroy()

        # 1) __RANDOM__ com Coluna "nome"
        r1 = self._add_param_row()
        r1.use_random.set(True)
        r1._toggle_param_name()
        r1.source_type.set("Coluna")
        r1._switch_source()
        r1.col_combo.set("nome")

        # 2) parameter_name "serie" com Coluna "telefone"
        r2 = self._add_param_row()
        r2.param_name_entry.insert(0, "serie")
        r2.source_type.set("Coluna")
        r2._switch_source()
        r2.col_combo.set("telefone")

        # 3) parameter_name "indicacao" com Literal "serie"
        r3 = self._add_param_row()
        r3.param_name_entry.insert(0, "indicacao")
        r3.source_type.set("Literal")
        r3._switch_source()
        r3.literal_var.set("serie")
    ...
    def _cadastrar_bm(self):
        ...
        top.title("Cadastrar BM")
        top.geometry("400x360")
        top.configure(bg="#111827")

        container = ttk.Frame(top, padding=14, style="Card.TFrame")
        ...
        nome = row("Nome:")
        phone = row("Phone Number ID:")
        token = row("Token:")
        token.configure(show="*")
        temps = row("Templates (vírgula):")

        def salvar():
            n = nome.get().strip()
            p = phone.get().strip()
            t = token.get().strip()
            ts = [x.strip() for x in temps.get().split(",") if x.strip()]
            if not n or not p or not t or not ts:
                messagebox.showwarning("Atenção", "Preencha todos os campos.")
                return
            bms = carregar_bms()
            bms[n] = {"phone_number_id": p, "token": t, "templates": ts}
            salvar_bms(bms)
            self._refresh_bms()
            self._log(f"BM {n} salva.")
            top.destroy()
    ...
    def _start_envio(self, random_mode):
        ...
        rows = [w for w in self.rows_container.inner.winfo_children() if isinstance(w, ParamRow)]
        if not rows:
            self._log("Adicione ao menos um parâmetro.")
            return

        # Thread pra não travar a UI
        t = threading.Thread(target=modo_envio, args=(self.selected_bm_obj, self.csv_df, rows, random_mode, self._threadsafe_log), daemon=True)
        t.start()
